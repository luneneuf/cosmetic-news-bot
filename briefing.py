...
# 회색지대: 임베딩만으론 애매(0.80~0.86)하지만 변별력 있는 고유명사를 2개 이상
# 공유하면 같은 사건으로 판정. 같은 기업의 '다른 사건'은 임베딩이 낮아 여기 안 걸림.
DEDUP_GRAY_THRESHOLD = float(os.environ.get("BRIEFING_DEDUP_GRAY_THRESHOLD", "0.80"))
...
_KO_PARTICLES = re.compile(
    r"(에서는|에서도|에게는|에게|에는|에도|에서|에만|에|을|를|이|가|은|는|의|과|와"
    r"|으로는|으로도|으로|로는|로도|로|도|만|까지|부터|이나|나|이라고|라고|이고|고)$"
)
...
def _distinctive_tokens(title: str) -> set[str]:
    ...
    같은 사건의 매체별 변형을 식별하는 핵심 신호는 고유명사(브랜드·인물·모델명)다.
    흔한 2~3글자 일반어(글로벌·마케팅·존재감 등)는 우연 일치로 오탐을 부르므로 제외:
      - 4글자 이상 한글 어근 (인물·브랜드·기업명: 최미나수, 애경산업, 올리브영)
      - 숫자를 포함한 영숫자 토큰 (모델·제품 코드: AGE20, SPF50)
    """
...
def dedup_by_vectors(
    ...
    gray_threshold: float = DEDUP_GRAY_THRESHOLD,
    min_shared_entities: int = 2,
) -> tuple[list[dict], list[dict]]:
    """벡터 유사도로 near-dup 제거 (순수 함수 — 오프라인 테스트 가능).

    앞에서부터 유지하며, 이미 유지된 항목과 중복이면 드롭. 중복 판정:
      - cosine >= threshold (강한 의미 유사) — 단독으로 중복, 또는
      - cosine >= gray_threshold AND 변별 고유명사를 min_shared_entities개 이상 공유
        (마케팅 앵글만 다른 같은 사건. 같은 기업 '다른 사건'은 cosine이 낮아 안 걸림)
    ...
    for it, v in zip(items, vectors):
        toks = _distinctive_tokens(it.get("title", ""))
        dup_of, sim = None, 0.0
        for k_it, k_v, k_toks in zip(kept, kept_vecs, kept_toks):
            s = _cosine(v, k_v)
            shared = len(toks & k_toks)
            if s >= threshold or (s >= gray_threshold and shared >= min_shared_entities):
                dup_of, sim = k_it, s
                break
        if dup_of is None:
            kept.append(it)
