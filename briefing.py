...
【카테고리 부여 — "어떤 키워드가 들어있나"가 아니라 "이 기사가 무엇에 대한 기사인가(핵심 주체)"로 판단】
- 자사: LAKA가 사건의 주인공.
- 경쟁사: 경쟁 브랜드가 주인공.
- 채널: 유통 채널(올리브영·시코르·세포라 등) **자체가 주인공**일 때만 — 채널의 출점·확장·실적·전략.
  ⚠️ 브랜드가 채널에 "입점"하는 기사는 채널이 아니라 그 **브랜드 기사**다(주체=브랜드).
     예: "오다다, 올리브영 미국점 입점" → 주체는 오다다 → (자사/경쟁사 아니면) 동향. 채널 아님.
     예: "K-뷰티 성지 美 상륙, 올리브영 1호점" → 주체는 올리브영 → 채널.
- 안전: 규제·회수·부작용·성분 논란이 주제.
- 동향: 시장·수출·M&A·IPO·트렌드 등 거시. **특정 트렌드가 주제면 동향**.
     예: "올영세일에 선 레이어링 확산" → 주제는 '선 레이어링' 트렌드 → 동향(세일·채널 아님).
- 자사·경쟁사가 아닌 일반/신생 브랜드 기사는 동향으로 분류.
- 요약도 그 핵심 주체·주제가 드러나게 쓴다(부차적 키워드에 끌려가지 말 것).
